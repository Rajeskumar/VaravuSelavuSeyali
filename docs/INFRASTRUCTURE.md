...
- **Cross-site cookie auth (resolved):** the frontend (`expense.cerebroos.com`) and backend were on different *sites* until the `trackspense-api.cerebroos.com` domain mapping (§4) shipped — the interim `AUTH_COOKIE_SAMESITE=none` fix was needed because Safari/WebKit's Intelligent Tracking Prevention blocks cross-site cookies outright regardless of `SameSite`, breaking login on every WebKit-based browser (Safari and, on iOS specifically, Chrome too — Apple mandates WebKit for all iOS browsers). Both apps are now same-site, `AUTH_COOKIE_SAMESITE` is back to `strict` in prod, and the frontend build points at `https://trackspense-api.cerebroos.com`. Full writeup: `docs/product_review&testing_report/TS-SEC-101-same-origin-auth-cookies.md`.
...
- **DB-backed refresh-token revocation** — implemented and tested (new `refresh_tokens` table, RFC 9700-style family-based rotation/revocation, 1-minute grace period for legitimate concurrent-refresh races), but **not yet committed to `main` / deployed** as of 2026-08-14 — it's sitting in the working tree. Until it ships, prod is still running the pre-existing in-memory set (doesn't survive a restart or span multiple Cloud Run instances). See `remediation-outcome.md` for the full design; treat this line, not that doc's "Resolved" note, as the source of truth on deploy status until the commit lands.
