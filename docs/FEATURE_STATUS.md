...
The two write tools (`create_expense` for personal expenses, `create_group_expense` for group expenses split equally among current members) let the chat agent log expenses on the user's behalf, not just answer questions about existing ones — extracted into standalone, LLM-free helper functions (`_create_personal_expense_from_agent`/`_create_group_expense_from_agent`) for deterministic testing (`tests/test_chat_create_expense_tools.py`, 14 tests). Both capture an optional `merchant_name` as a separate searchable field rather than folding it into the description. `create_group_expense` additionally accepts a free-text `paid_by` — unset (or "me") defaults to the caller, otherwise resolved via case-insensitive substring match against the group's member display names (`_resolve_payer`), with a clear error message back to the LLM (and thus the user) if no member matches. Update and delete are explicitly **not** implemented yet — scoped out deliberately, since an LLM mutating or removing the wrong existing record has no undo, unlike a create mistake which is just a new row to delete.
...
| Chat can call tools for item/merchant data | ✅ Built (LangGraph agent, 5 tools: 3 read + `create_expense`/`create_group_expense`) |
...
| Spec | Status | Notes |
|:---|:---|:---|
| [TS-GRP-113 — Shares and Adjustments](features/tickets/TS-GRP-113-shares-adjustments.md) | ✅ Built (Backend) | Added full support in `split_engine.py` for proportional share splitting and fixed offset adjustments. Validations implemented. |
| [TS-GRP-114 — Multi-Payer Expenses](features/tickets/TS-GRP-114-multi-payer.md) | ✅ Built (Backend) | Removed single-payer limit, implemented validations, refactored balances math to properly calculate debt distributions proportionally. |
| [TS-GRP-115 — Itemized Group Expenses](features/tickets/TS-GRP-115-itemized-expenses.md) | ✅ Built (Backend) | Created `expense_item_splits` table (Alembic migration `b221e1af9b3d`), wrote `resolve_itemized_split` mathematical engine, exposed via `POST /api/v1/groups/{group_id}/expenses/itemized`. |
| [TS-GRP-118 — Simplify Debts](features/tickets/TS-GRP-118-simplify-debts-and-group-settings.md) | ✅ Built | Added `simplify_debts` and `default_split` settings to groups, updated `BalanceService`, built Web and Mobile UI settings sheets/dialogs. |
| [TS-GRP-119 — Activity Feed](features/tickets/TS-GRP-119-activity-feed.md) | ✅ Built | Created `group_activity` table, wired into `ActivityService`, and built Web and Mobile `ActivityList` tabs on the group detail page. |
| [TS-GRP-120 — Recurring Group Expenses](features/tickets/TS-GRP-120-recurring-group-expenses.md) | ✅ Built | Added `group_id` and `split_config` to `RecurringTemplate`, updated backend endpoints, built tests, added SplitEditor functionality to Recurring UI in Web and Mobile. |
| [TS-GRP-121 — Personal to Group Conversion](features/tickets/TS-GRP-121-personal-group-conversion.md) | ✅ Built | Added capability to override split strategy for recurring templates and handle group membership validation in recurring executions. |
...
> **Note:** this table stops at `TS-DES-110` and was never updated for the later `TS-DES-111`–`112` and `TS-DES-201`–`210` "Redesign v2" tickets (all already `✅ Implemented` per their own files) — a pre-existing gap, not introduced here.
>
> **2026-07-14, untracked (no ticket file):** Quick Capture unified into a single fast-entry surface across mobile and desktop — a shared `QuickCaptureContext`/`QuickCaptureSheet` (mobile bottom sheet + numeric keypad, desktop centered dialog), reachable from the header "+ New expense" button, the mobile FAB, `ExpensesPage`'s Add Expense button, and a group's own "+ Add expense" button (pre-scoped to that group). This replaced the old per-page `AddExpenseForm` dialogs as the *create* path everywhere (`AddExpenseForm` itself is unchanged and still used for editing). Desktop also gained the header type-to-log bar (`HeaderQuickLog`, reusing the existing regex parser + `useLogExpense`) that mobile already had. The dashboard hero (`TrueTotalHero`) gained a "Net with people" figure + My-expenses/I-paid lens toggle on both breakpoints (desktop previously had neither), and `OverviewTab` (Analysis) gained an "include group shares" toggle. Also fixed: the Dashboard "Recent" feed not refreshing after logging an expense without navigating away and back.
