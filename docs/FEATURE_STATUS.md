...
| [TS-GRP-105 — Settlements](features/tickets/TS-GRP-105-settlements.md) | ✅ Built | Record/list/undo settlements between group members; never touches `expenses` or the analysis cache (rule TS-GRP-R2), verified by a test that checks spend totals before/after. Consumed by TS-GRP-104's `BalanceService` next. |
