**Status:** 🚧 Partial — verified 2026-07-04. This is a strategic vision document; most of its specific proposals were **not** the path actually taken (see [FEATURE_STATUS.md](../FEATURE_STATUS.md) §3 for the full comparison).

**What shipped:** the core insight — pre-calculate and aggregate instead of feeding raw line items to the LLM — was implemented, but via the simpler schema proposed in [item-level-ai-analyst.md](ai_analyst/item-level-ai-analyst.md) (`item_insights`/`merchant_insights` tables), not this document's Medallion architecture (`Vendors`/`Purchases`/`Canonical_Products`/`Purchase_Line_Items` — none of these tables exist). The chat engine uses a LangGraph tool-calling agent, not the Text-to-SQL agent proposed here.

**Not built:** Text-to-SQL agent, personal inflation rate calculator, vector-embedding/semantic entity resolution (matching is plain string/normalized-name only), dual-path hallucination validation, "zombie expense"/micro-habit clustering, granular tax categorization, proactive store-arbitrage notifications (the underlying price-comparison data exists and is queryable, but nothing surfaces it proactively).
