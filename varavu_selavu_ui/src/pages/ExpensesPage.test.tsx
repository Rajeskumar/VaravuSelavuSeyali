...
test('shows expenses and opens Quick Capture', async () => {
  ...
  jest.spyOn(configApi, 'getConfig').mockResolvedValue({ groups_enabled: false, entity_resolution_enabled: false, budgets_enabled: true, card_coach_enabled: false });
  renderPage();
...
test('scope filter switches the queried data and shows the group badge column', async () => {
  ...
  jest.spyOn(configApi, 'getConfig').mockResolvedValue({ groups_enabled: true, entity_resolution_enabled: false, budgets_enabled: true, card_coach_enabled: false });
