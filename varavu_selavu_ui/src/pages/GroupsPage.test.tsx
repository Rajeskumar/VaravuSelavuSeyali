...
test('renders the user\'s groups', async () => {
  jest.spyOn(api, 'listGroups').mockResolvedValue([
    { group_id: 'g1', name: 'Apartment 4B', group_type: 'home', currency: 'USD', member_count: 3, my_balance: 42.17, status: 'active', archived_at: null, deleted_at: null },
  ]);
  renderPage();
  await screen.findByText(/Apartment 4B/);
...
test('shows an empty state when the user has no groups', async () => {
  ...
  await screen.findByText(/No active groups yet/);
