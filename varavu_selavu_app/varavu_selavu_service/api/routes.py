...
from varavu_selavu_service.services.chat_service import call_ollama
import pandas as pd
import time
...
def analysis_chat(request: ChatRequest, response: Response = None):
    """
    Accepts a chat query and returns a response generated by the local
    Ollama instance.  The query is sent together with the analysis data
    ...
    # Pass the query + analysis to Ollama
    chat_response = call_ollama(query=request.query, analysis=analysis_result)
