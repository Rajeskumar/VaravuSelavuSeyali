...
> **Note:** Detailed architectural studies are available:
> 1. `FEASIBILITY_STUDY.md`: Analyzes the feasibility of the "Bring Your Own Google Sheet" model.
> 2. `ALTERNATIVE_PRIVACY_ARCHITECTURE.md`: Proposes a "Local-First" architecture with end-to-end encryption for maximum privacy *and* performance.
...
However, if the strategic goal is to prioritize **User Privacy** over raw performance, you have two alternative paths:
1. **Bring Your Own Google Sheet:** (See `FEASIBILITY_STUDY.md`) - Feasible but slow.
2. **Local-First with E2E Encryption:** (See `ALTERNATIVE_PRIVACY_ARCHITECTURE.md`) - **Recommended.** This approach offers the best of both worlds: extreme privacy (the server sees only encrypted blobs) and extreme performance (local SQLite).
